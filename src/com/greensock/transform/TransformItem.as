 ...
 */
	public class TransformItem extends EventDispatcher {
		/** @private **/
		public static const VERSION:Number = 1.932;
		...
		public function TransformItem($targetObject:DisplayObject, $vars:Object) {
			if (TransformManager.VERSION < 1.92) {
...
//---- SCALE -------------------------------------------------------------------------------------------
		 ...
		 * @param customBounds Normally, <code>fitInsideBounds()</code> uses the TargetItem's <code>bounds</code> property, but if you'd like it to use a custom Rectangle instead, define it here. Use <code>null</code> in order to apply the TransformItem's normal bounds that were likely set through its TransformManager.
		 */
		public function fitInsideBounds(move:Boolean=true, updateFirst:Boolean=true, customBounds:Rectangle=null):void {
			...
			if (_target.parent && mBounds != null) {
				...
				if (!mBounds.containsRect(bounds) && bounds.width != 0) {
					...
					var w:Number = Math.min(bounds.width, mBounds.width - (xMin - mBounds.x));
					var h:Number = Math.min(bounds.height, mBounds.height - (yMin - mBounds.y));
...
//---- GETTERS / SETTERS --------------------------------------------------------------------------------
		...
		public function set constrainScale($b:Boolean):void {
			_constrainScale = $b;
			var b:Rectangle = _targetObject.getBounds(_targetObject);
			var m:Matrix = _targetObject.transform.matrix;
			var sx:Number = MatrixTools.getScaleX(m, _flipX);
			var sy:Number = MatrixTools.getScaleY(m, _flipX);
			if (b.width / b.height != sx / sy) {
				if (Math.abs(sx) < Math.abs(sy)) {
					_targetObject.scaleY = _targetObject.scaleX;
				} else {
					_targetObject.scaleX = _targetObject.scaleY;
				}
				if (_selected) {
					reposition();
				}
		...
		public function set maxWidth(value:Number):void {
			...
			var m:Matrix = _targetObject.transform.matrix;
			var w:Number = _targetObject.getBounds(_targetObject).width * Math.sqrt(m.a * m.a + m.b * m.b);
			if (w > _maxWidth) {
		...
		public function set maxHeight(value:Number):void {
			...
			var m:Matrix = _targetObject.transform.matrix;
			var h:Number = _targetObject.getBounds(_targetObject).height * Math.sqrt(m.c * m.c + m.d * m.d);
			if (h > _maxHeight) {
